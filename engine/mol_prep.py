...
def pdb_filter_components(pdb_str: str, kept_components: list[dict]) -> tuple[str, str]:
    """Keep only atoms that belong to the given component dicts.
    Each component dict has: type, chain, resname, resseq.
    - Protein  → keep by chain (ATOM records)
    - Water    → keep all WATER_RESNAMES HETATM
    - Metal Ion→ keep all metal HETATM
    - Ligand   → keep by exact (chain, resname, resseq)
    """
    keep_prot_chains = set()
    keep_water       = False
    keep_metals      = False
    keep_ligands     : set[tuple[str, str, str]] = set()

    for comp in kept_components:
        t = comp['type']
        if t == 'Protein':
            keep_prot_chains.add(comp['chain'].upper())
        elif t == 'Water':
            keep_water = True
        elif t == 'Metal Ion':
            keep_metals = True
    ...
    for line in pdb_str.splitlines(keepends=True):
        ...
        keep = False
        if rec == 'ATOM':
            keep = chain in keep_prot_chains
        elif resname in WATER_RESNAMES:
            keep = keep_water
        elif elem in METAL_ELEMENTS or resname in METAL_ELEMENTS:
            keep = keep_metals
        else:  # ligand / other HETATM
            keep = (chain, resname, resseq) in keep_ligands
...
def rdkit_add_gasteiger_charges(pdb_str: str) -> tuple[str, str]:
    """Compute Gasteiger charges; store in B-factor column of PDB."""
    ...
    for line in pdb_str.splitlines(keepends=True):
        a = _parse_atom_line(line)
        if a and atom_idx < mol.GetNumAtoms():
            charge = mol.GetAtomWithIdx(atom_idx).GetDoubleProp('_GasteigerCharge')
            # Pack charge into B-factor field (cols 60-66)
            line = f"{line[:60]}{charge:6.3f}{line[66:]}"
            atom_idx += 1
        lines.append(line)
    return ''.join(lines), f"Gasteiger charges computed for {atom_idx} atoms (stored in B-factor)."
