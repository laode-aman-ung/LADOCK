...
from ladock.desktop.core.tool_paths import (
    autodock_gpu_runnable,
    preferred_tool_candidates,
    mgltools_candidates,
    resolve_tool_path,
)

# Tools that are bundled (always available inside bin/<platform>/)
...
_TOOLS: list[ToolInfo] = [
    ...
    # ADFRsuite is NOT bundled: it is ~500 MB and the release build prunes it
    # (packaging/build_release.py --with-adfrsuite opts back in). Label them as
    # optional so a missing suite reads as "not installed", not "broken install".
    ToolInfo("adfr",         "ADFR [optional]",            "adfr",
             "--version",
             "AutoDockFR docking engine — install ADFRsuite separately, or "
             "set its path in Settings"),
    ToolInfo("agfr",         "AGFR [optional]",            "agfr",
             "--version",
             "AutoGrid FR target preparation — install ADFRsuite separately, or "
             "set its path in Settings"),
    ToolInfo("mgltools",     "MGLTools [bundled]",         "prepare_receptor4.py",
             "-h",
             "MGLTools suite — bundled in bin/linux/MGLTools-1.5.6/"),
]
...
def _find_binary(binary: str, key: str, use_wsl_backend: bool = False) -> Optional[str]:
    """Look up a binary for the active platform (host, or Linux under hybrid WSL).
    Cross-platform binaries are not matched, so a Linux-only tool is unavailable
    on native Windows unless hybrid WSL mode is enabled."""
    import shutil
...
def _find_mgltools(use_wsl_backend: bool = False) -> Optional[str]:
    """Try to find MGLTools installation directory — bundled bin/ first."""
    for candidate in mgltools_candidates(use_wsl_backend):
        c = Path(candidate)
...
def _apply_found(t: ToolInfo, found: str, use_wsl_backend: bool = False) -> None:
    """Populate a ToolInfo from a located binary, applying the CUDA gate for
    AutoDock-GPU (a present binary whose CUDA runtime is missing is reported as
    unavailable with an explanatory reason)."""
    t.found_path = found
    if t.key == "autodock_gpu" and not autodock_gpu_runnable(found, use_wsl_backend):
        t.available = False
        t.version = "CUDA runtime not found"
    else:
        t.available = True
        # Version probing executes the binary. Under hybrid WSL the bundled
        # binaries are Linux ELF and can't run on the Windows host, so skip it.
        t.version = "found" if use_wsl_backend else _get_version(found, t.version_flag)


def detect_all(use_wsl_backend: bool = False) -> dict[str, ToolInfo]:
    """Detect all tools and return mapping key → ToolInfo."""
    results: dict[str, ToolInfo] = {}
    for tool in _TOOLS:
        t = ToolInfo(**tool.__dict__)   # shallow copy
        if t.key == "mgltools":
            d = _find_mgltools(use_wsl_backend)
            if d:
                t.found_path = d
                t.available = True
                t.version = "found"
        else:
            found = _find_binary(t.binary, t.key, use_wsl_backend)
            if found:
                _apply_found(t, found, use_wsl_backend)
        results[t.key] = t
    return results


def detect_one(key: str, use_wsl_backend: bool = False) -> Optional[ToolInfo]:
