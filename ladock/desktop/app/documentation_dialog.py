...
<p>Binaries ship per platform under <code>ladock/bin/&lt;platform&gt;/</code>
(<code>windows</code>, <code>linux</code>, <code>mac</code>). The Linux folder also holds
AutoDock4/AutoGrid4, AutoDock-GPU, ADFRsuite and MGLTools. They are not part of
the pip install — fetch them once with <code>ladock-fetch-binaries</code>, or point
<code>LADOCK_BIN</code> at an existing copy.</p>

<h3>Requirements</h3>
<p><code>PySide6</code>, <code>numpy</code>, <code>pandas</code>, <code>openpyxl</code>,
<code>rdkit</code>, and <code>meeko</code> (+ <code>gemmi</code>) — all installed by
<code>pip install ladock</code>, which provides both <code>ladock-desktop</code> (this
app) and <code>ladock-cli</code> (the rule-based docking agent). The embedded 3D
viewer needs no extra package: QtWebEngine ships inside the PySide6 wheel.</p>
"""
