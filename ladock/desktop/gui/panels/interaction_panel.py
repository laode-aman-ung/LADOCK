...
from ladock.desktop.core.wsl_backend import prepare_subprocess, resolve_wsl_python, wsl_available
...
class _DiagramCanvas(QWidget):
    ...
    def _draw_bipartite(self, p: QPainter):
        ...
        for lk, (lx, ly) in lig_pos.items():
            rim  = QColor(ITYPE_COLORS.get(lig_map[lk][0].itype, "#d29922"))
            fill = self._node_fill(rim)
    ...
    _RES_DEFAULT = '#90a4ae'
    _ELEM_COLORS = {
        'O': '#f85149', 'N': '#58a6ff', 'S': '#f9e2af',
        'F': '#3fb950', 'Cl': '#3fb950', 'Br': '#d29922', 'P': '#d29922',
    }
    ...
    def _render_smiles_depiction_local(self, smiles: str, width: int, height: int,
                                       white_bg: bool = False) -> tuple[str, dict[str, tuple[float, float]]]:
        try:
            from ladock.desktop.core.render_smiles_svg import render_smiles_depiction
