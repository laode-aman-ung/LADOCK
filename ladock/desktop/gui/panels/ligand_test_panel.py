...
from ladock.desktop.gui.widgets.common import SectionLabel, HDivider, apply_docking_tooltips
...
from ladock.desktop.core.ligand_smiles import smiles_from_structure
from ladock.desktop.core.wsl_backend import prepare_subprocess, wsl_available
from ladock.desktop.engine.native_prep import meeko_available, native_prepare_receptor
...
class _LigandDockingWorker(QObject):
    ...
    def _convert_ligand(self, lig_path: str, tmp: str, idx: int) -> List[dict]:
        ...
        """
        from ladock.desktop.core.ligand_importer import expand_to_pdbqt
        p        = self._p
    ...
    def _iter_convert_ligand(self, lig_path: str, tmp: str, idx: int):
        ...
        if ext in ('.smi', '.smiles', '.txt', '.csv', '.tsv', '.xlsx', '.xls', '.sdf', '.mol2'):
            from ladock.desktop.core.ligand_importer import (
                count_molecules,
    ...
    def _build_input_smiles_map(self, lig_path: str,
        ...
        ext = os.path.splitext(lig_path)[1].lower()
        try:
            from ladock.desktop.core.ligand_importer import _find_smiles_and_name_cols, _looks_like_smiles
        except Exception:
...
class LigandTestPanel(QWidget):
    ...
    def _build_main_content(self, root):
        ...
        from ladock.desktop.engine.tool_detector import INSTALL_URLS, PIP_INSTALL, BUNDLED_KEYS
    ...
    def _refresh_ligand_list(self, subdir: str = "ligand_input"):
        ...
        if not os.path.isdir(lig_dir):
            return
        from ladock.desktop.core.ligand_importer import SUPPORTED_EXTENSIONS, count_molecules
    ...
    def _gate_feature_controls(self):
        ...
        from ladock.desktop.core.docking_features import any_supports_flex, any_supports_mlsd
    ...
    def _extract_native_ligand_for_queue(self) -> str:
        ...
        if not native_comp:
            return ''
        try:
            from ladock.desktop.gui.panels.native_redocking_panel import extract_pdb_component
            import tempfile, os
    ...
    def _detect_tools(self):
        from ladock.desktop.engine.tool_detector import detect_all
    ...
    def _update_sf_availability(self):
        ...
        import os
        from ladock.desktop.core.tool_paths import tool_available, autodock_gpu_runnable
    ...
    def _enqueue_pending(self, params: dict):
        ...
        from ladock.desktop.core.job_scheduler import DockingJob, JobStatus
